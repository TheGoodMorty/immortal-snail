...
public final class PlayerFirstJoinHandler {
    ...
    public static void register() {
        ServerPlayConnectionEvents.JOIN.register((handler, sender, server) -> {
            ...
            UUID id = player.getPlayerConfigEntry().id();
        ...
        ServerPlayConnectionEvents.DISCONNECT.register((handler, server) -> {
            UUID id = handler.getPlayer().getPlayerConfigEntry().id();
        ...
        ServerTickEvents.END_SERVER_TICK.register(server -> {
            ...
            for (UUID id : toOpen) {
                ...
                if (player != null && player.networkHandler.isConnectionOpen()) {
                    try {
                        player.openHandledScreen(new com.yourname.immortalsnail.player.BargainScreenHandler.Factory());
                    } catch (Exception e) {
     ...
     */
    public static void onBargainSubmitted(MinecraftServer server, ServerPlayerEntity player, ItemStack[] shulkerA, ItemStack[] shulkerB) {
        UUID id = player.getPlayerConfigEntry().id();
        ...
        boolean gaveA = player.getInventory().insertStack(boxA);
        boolean gaveB = player.getInventory().insertStack(boxB);
        ...
        if (!gaveA || !gaveB) {
            ...
            if (!gaveA) player.dropItem(boxA, false);
            if (!gaveB) player.dropItem(boxB, false);
        }
    }

    private static ItemStack fillShulkerBox(ItemStack[] contents, ServerPlayerEntity player) {
        ...
        box.set(DataComponentTypes.CONTAINER,
                ContainerComponent.fromStacks(containerItems));
