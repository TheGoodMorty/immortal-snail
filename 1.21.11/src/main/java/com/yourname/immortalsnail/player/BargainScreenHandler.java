 ...
 */
public class BargainScreenHandler extends ScreenHandler {
    ...
    private final SimpleInventory shulkerA = new SimpleInventory(27);
    private final SimpleInventory shulkerB = new SimpleInventory(27);

    public BargainScreenHandler(int syncId, PlayerInventory playerInventory) {
    ...
    public ItemStack[] getShulkerAContents() {
        ...
        for (int i = 0; i < 27; i++) {
            ItemStack s = shulkerA.getStack(i);
    ...
    public ItemStack[] getShulkerBContents() {
        ItemStack[] out = new ItemStack[27];
        for (int i = 0; i < 27; i++) {
            ItemStack s = shulkerB.getStack(i);
            out[i] = s.isEmpty() ? null : s.copy();
        }
        return out;
    }
    ...
    private boolean isBlacklisted(ItemStack stack) {
        var key = net.minecraft.registry.Registries.ITEM.getKey(stack.getItem());
     ...
     * Implements {@link NamedScreenHandlerFactory}.
     */
    public static class Factory implements NamedScreenHandlerFactory {
        @Override
        public Text getDisplayName() {
            return Text.literal("The Bargain");
        }
