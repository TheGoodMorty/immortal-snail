...
public final class StarterGear {
    ...
    public static void giveStarter(ServerPlayerEntity player) {
    ...
    private static ItemStack makeShulkerBox(ItemStack[] contents) {
        ...
        shulker.set(DataComponentTypes.CONTAINER,
                net.minecraft.component.type.ContainerComponent.fromStacks(containerItems));
        return shulker;
    }
    ...
    private static ItemStack maxMace(MinecraftServer server) {
        ...
        Identifier densityId = Identifier.of("minecraft", "density");
        Identifier breachId = Identifier.of("minecraft", "breach");
        Identifier windBurstId = Identifier.of("minecraft", "wind_burst");
        var registryAccess = server.getRegistryManager();
        var enchReg = registryAccess.getOrThrow(net.minecraft.registry.RegistryKeys.ENCHANTMENT);
        enchReg.getEntry(densityId).ifPresent(h -> addEnchantHolder(s, h, 5));
        enchReg.getEntry(breachId).ifPresent(h -> addEnchantHolder(s, h, 4));
        enchReg.getEntry(windBurstId).ifPresent(h -> addEnchantHolder(s, h, 3));
        return s;
    }
    ...
    private static void addEnchant(MinecraftServer server, ItemStack stack, RegistryKey<Enchantment> key, int level) {
        var registryAccess = server.getRegistryManager();
        var enchReg = registryAccess.getOrThrow(net.minecraft.registry.RegistryKeys.ENCHANTMENT);
        var entryOpt = enchReg.getEntry(key.getValue());
        entryOpt.ifPresent(holder -> addEnchantHolder(stack, holder, level));
    }

    private static void addEnchantHolder(ItemStack stack, RegistryEntry<Enchantment> holder, int level) {
        EnchantmentHelper.apply(stack, builder -> builder.add(holder, level));
    }
}
