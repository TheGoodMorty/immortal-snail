...
/**
 * The Snail â€” slow-moving entity that chases the nearest non-op player.
 *
 * Movement is driven manually in tick() via setVelocity() because the
 * vanilla pathfinder doesn't work well with the snail's tiny 0.25Ã—0.25 size.
 *
 * Spider-like wall climbing: when the snail collides with a wall, it
 * sets climbing and slowly moves upward. Falls via gravity.
 */
public class SnailEntity extends SpiderEntity {

    private static final TrackedData<String> DATA_MODE =
            DataTracker.registerData(SnailEntity.class, TrackedDataHandlerRegistry.STRING);
    private static final TrackedData<Integer> DATA_BREAK_TICKS =
            DataTracker.registerData(SnailEntity.class, TrackedDataHandlerRegistry.INTEGER);
    private static final TrackedData<Integer> DATA_SURFACE_POSE =
            DataTracker.registerData(SnailEntity.class, TrackedDataHandlerRegistry.INTEGER);
    ...
    private Vec3d lastToTarget = null;
    /**
     ...
     */
    private Vec3d lastMoveDir = null;
    ...
    private Vec3d lastWallNormal = null;

    public SnailEntity(EntityType<? extends SpiderEntity> entityType, World level) {
        super(entityType, level);
        this.setSilent(true);
        this.setCustomName(Text.literal("The Snail"));
    ...
    public static DefaultAttributeContainer.Builder createAttributes() {
        return net.minecraft.entity.mob.HostileEntity.createHostileAttributes()
                .add(net.minecraft.entity.attribute.EntityAttributes.MAX_HEALTH, 1.0)
                .add(net.minecraft.entity.attribute.EntityAttributes.MOVEMENT_SPEED, 0.08)
                .add(net.minecraft.entity.attribute.EntityAttributes.KNOCKBACK_RESISTANCE, 1.0)
                .add(net.minecraft.entity.attribute.EntityAttributes.FOLLOW_RANGE, 64.0);
    }

    @Override
    protected void initDataTracker(DataTracker.Builder builder) {
        super.initDataTracker(builder);
        builder.add(DATA_MODE, SnailMode.TRAVELING.name());
        builder.add(DATA_BREAK_TICKS, 0);
        builder.add(DATA_SURFACE_POSE, POSE_FLOOR);
    }

    public SnailMode getMode() {
        return SnailMode.valueOf(this.dataTracker.get(DATA_MODE));
    }

    public void setMode(SnailMode mode) {
        this.dataTracker.set(DATA_MODE, mode.name());
    }

    public int getBreakTicks() {
        return this.dataTracker.get(DATA_BREAK_TICKS);
    }

    public void setBreakTicks(int ticks) {
        this.dataTracker.set(DATA_BREAK_TICKS, ticks);
    }

    public int getSurfacePose() {
        return this.dataTracker.get(DATA_SURFACE_POSE);
    }

    public void setSurfacePose(int pose) {
        this.dataTracker.set(DATA_SURFACE_POSE, pose);
    }
     ...
     * never move via setVelocity. Instead we bank the per-tick distance in
    ...
    public void tick() {
        ...
        if (!this.getEntityWorld().isClient()) {
            updateCling((ServerWorld) this.getEntityWorld());
        }

        super.tick();

        if (!this.getEntityWorld().isClient()) {
            ServerWorld level = (ServerWorld) this.getEntityWorld();
    ...
    public void travel(Vec3d movementInput) {
        if (!this.getEntityWorld().isClient() && this.clingingToWall) {
     ...
     */
    private void updateCling(ServerWorld level) {
        if (this.isOnGround()) {
            this.clingingToWall = false;
            return;
        }
        ServerPlayerEntity target = findClosestPlayer(level, this.getEntityPos());
        if (target == null) {
            this.clingingToWall = false;
            return;
        }
        ...
        Vec3d wallNormal = findWallNormal(level);
        if (wallNormal != null && dy <= 0.3) {
            ...
            Vec3d toTarget = target.getEntityPos().subtract(this.getEntityPos());
            if (toTarget.normalize().dotProduct(wallNormal) > 0.3) {
                this.clingingToWall = false;
                return;
            }
        }
     ...
     */
    private void stepTowardTarget(ServerWorld level) {
        ServerPlayerEntity target = findClosestPlayer(level, this.getEntityPos());
        ...
        Vec3d rawDir = toTarget.multiply(1.0 / dist);
        ...
        Vec3d normal = contactNormal(level);
        Vec3d moveDir;
        ...
        if (normal == null || normal.y > 0.5) {
            ...
            if (rawDir.y < -0.3) {
                ...
                Vec3d tangent = projectOntoPlane(rawDir, normal);
                if (tangent.lengthSquared() < 0.1) tangent = horizontal(rawDir);
                else tangent = tangent.normalize();
                moveDir = tangent.add(normal.multiply(-0.4)).normalize();
            }
        } else {
            // Wall: climb up or crawl sideways, pressing in to keep the grip.
            double dy = target.getY() - this.getY();
            double dot = rawDir.dotProduct(normal);
        ...
        this.move(MovementType.SELF, moveDir.multiply(step));
        if (before.distanceTo(this.getEntityPos()) < step * 0.5) {
     ...
     */
    private void onBlocked(ServerWorld level, Vec3d dir, double step) {
        Vec3d lunge = new Vec3d(dir.x, 0.4, dir.z).normalize().multiply(step);
        this.move(MovementType.SELF, lunge);
    }
    ...
    private static Vec3d horizontal(Vec3d v) {
        ...
        return h.lengthSquared() < 1.0e-6 ? Vec3d.ZERO : h.normalize();
    }
     ...
     */
    private void tickStuckDetection(ServerWorld level) {
        // Free mid-air with no contact and no progress is fine â€” just
        ...
        // On a surface but not making forward progress â€” increment.
        stuckTicks++;
        ...
        level.syncWorldEvent(null, 2001, target,
                net.minecraft.block.Block.getRawIdFromState(state));
        com.yourname.immortalsnail.ImmortalSnail.LOGGER.info(
     ...
     */
    private BlockPos findStuckBlock(ServerWorld level) {
        ...
        Vec3d facingDir;
        if (lastMoveDir != null) {
            facingDir = new Vec3d(lastMoveDir.x, 0.0, lastMoveDir.z);
        } else {
            float yaw = this.getYaw();
            facingDir = new Vec3d(-Math.sin(Math.toRadians(yaw)), 0.0, Math.cos(Math.toRadians(yaw)));
        }
        ...
        double bodyY = this.getY() + this.getHeight() * 0.5;
        for (double y : new double[]{this.getY() + 0.1, bodyY, this.getY() + this.getHeight() + 0.05}) {
            BlockPos ahead = BlockPos.ofFloored(new Vec3d(
        ...
        Box box = this.getBoundingBox().expand(0.05);
        for (int dx = -1; dx <= 1; dx++) {
            for (int dy = 0; dy <= 1; dy++) {
                for (int dz = -1; dz <= 1; dz++) {
                    BlockPos probe = BlockPos.ofFloored(new Vec3d(
                            this.getX() + dx * 0.5,
                            this.getY() + dy * 0.5,
                            this.getZ() + dz * 0.5));
                    if (box.intersects(new Box(probe.getX(), probe.getY(), probe.getZ(),
    ...
    private boolean wallAhead(ServerWorld level, Vec3d dir) {
        double probe = 0.35;
        double px = this.getX() + dir.x * probe;
        double pz = this.getZ() + dir.z * probe;
        BlockPos low = BlockPos.ofFloored(px, this.getY() + 0.1, pz);
        BlockPos high = BlockPos.ofFloored(px, this.getY() + this.getHeight() * 0.9, pz);
     ...
     */
    private Vec3d contactNormal(ServerWorld level) {
        ...
        if (this.isOnGround()) {
            ...
            BlockPos below = BlockPos.ofFloored(new Vec3d(this.getX(), this.getY() - 0.05, this.getZ()));
        ...
        if (!this.isOnGround() && touchingCeiling(level)) {
            return new Vec3d(0.0, -1.0, 0.0);
        }
     ...
     * normal, or null. Uses vanilla's own contact tool â€” a collision-shape
     ...
     */
    private Vec3d findWallNormal(ServerWorld level) {
        double maxDist = this.getWidth() * 0.5 + WALL_GRIP_TOLERANCE;
        double[] probeHeights = new double[]{
                this.getY() + 0.05,
                this.getY() + this.getHeight() * 0.5};
        for (Direction d : Direction.Type.HORIZONTAL) {
            Vec3d dir = Vec3d.of(d.getVector());
            for (double h : probeHeights) {
                Vec3d start = new Vec3d(this.getX(), h, this.getZ());
                ...
                if (hit.getType() == HitResult.Type.BLOCK && hit.getSide() == d.getOpposite()) {
                    return Vec3d.of(hit.getSide().getVector());
                }
            }
        }
        return null;
     ...
     * {@link #findWallNormal}.
     */
    private boolean touchingCeiling(ServerWorld level) {
        ...
        BlockHitResult hit = level.raycast(new RaycastContext(
                center,
                center.add(0.0, this.getHeight() * 0.5 + WALL_GRIP_TOLERANCE, 0.0),
                RaycastContext.ShapeType.COLLIDER,
                RaycastContext.FluidHandling.NONE,
                this));
        return hit.getType() == HitResult.Type.BLOCK && hit.getSide() == Direction.DOWN;
    }
    ...
    private static Vec3d projectOntoPlane(Vec3d v, Vec3d normal) {
        ...
        return v.subtract(normal.multiply(d));
    }
     ...
     */
    private void orientToSurface(ServerWorld level) {
        Vec3d normal = contactNormal(level);
        if (normal == null) {
            // Free air â€” flatten out and face the travel direction.
            this.setPitch(0.0f);
        ...
        if (normal.y > 0.5) {
            ...
            // Ceiling â€” flip upside down; the yaw keeps facing the travel
            // direction so the head leads horizontally along the ceiling.
            this.setPitch(180.0f);
            this.setSurfacePose(POSE_CEILING);
            faceTravelDirection();
        } else {
            // Wall â€” pick the pose whose head points where the snail is
            // about to go:
            ...
            Vec3d travel = lastMoveDir != null ? lastMoveDir : Vec3d.ZERO;
     ...
     * targets â€” singleplayer auto-OPs the host, and the original op-filter
     * would have meant the mod never engaged the host at all.
     */
    public static ServerPlayerEntity findClosestPlayer(ServerWorld level, Vec3d from) {
        ServerPlayerEntity nearest = null;
        double nearestDist = Double.MAX_VALUE;
        for (ServerPlayerEntity p : level.getPlayers()) {
            if (p.isSpectator()) continue;
            double d = p.getEntityPos().squaredDistanceTo(from);
            if (d < nearestDist) {
                nearestDist = d;
                nearest = p;
            }
    ...
    public ServerPlayerEntity findNearestPlayer(ServerWorld level) {
        return findClosestPlayer(level, this.getEntityPos());
    }

    public boolean canBreakBlock(BlockState state) {
        SnailConfig.Snail cfg = SnailConfig.get().snail;
        if (!cfg.canBreakBlocks) return false;
        var keyOpt = net.minecraft.registry.Registries.BLOCK.getKey(state.getBlock());
